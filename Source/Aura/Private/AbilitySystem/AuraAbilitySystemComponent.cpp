	...
	for(const FAuraAbilityInfo& Info : AbilityInfo->AbilityInformation)
	{
		...
		if(Level < Info.LevelRequirement) continue;
			if(GetSpecFromAbilityTag(Info.AbilityTag) == nullptr)
			{
				...
				GiveAbility(AbilitySpec);
				// force an update using this
				MarkAbilitySpecDirty(AbilitySpec);
				// RPC to update the client side
	...
	if(FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if(bStatusValid)
		{
			...
			if(!SlotIsEmpty(Slot)) // is an ability in this slot, deactivate and clear the slot
			{
				...
				if(SpecWithSlot)
				{
					// is new ability the same that is in the slot? if yes, return early
...
bool UAuraAbilitySystemComponent::GetDescByAbilityTag(const FGameplayTag& AbilityTag, FString& OutDesc, FString& OutNextLevelDesc)
{
	...
	{
		OutDesc = UAuraGameplayAbility::GetLockedDesc(AbilityInfo->FindAbilityInfoForTag(AbilityTag).LevelRequirement);
	}	
